...
TEST_CASE("Strip strings", "[string]"){
  SECTION("Narrow strings") {
    ...
    SECTION("Leading and trailing white-space is removed") {
      const auto [str, expected] = GENERATE(table<const char*, std::string>({
        {" space ", "space"s},
        {"\ttab\t", "tab"s},
        {"\rcarriage return\r", "carriage return"}, 
        {"\vvertical tab\v", "vertical tab"}
      }));
  ...
  SECTION("Wide strings") {
    ...
    const auto [str, expected] = GENERATE(table<const wchar_t*, std::wstring>({
      {L" space ", L"space"s},
      {L"\ttab\t", L"tab"s},
      {L"\rcarriage return\r", L"carriage return"}, 
      {L"\vvertical tab\v", L"vertical tab"}
    }));
