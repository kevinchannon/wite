...
TEST_CASE("From hex chars tests", "[binascii]") {
  ...
  SECTION("uint16_t") {
    SECTION("works for valid string") { REQUIRE(uint16_t{0xDCBA} == binascii::from_hex_chars<uint16_t>("BadC")); }
  ...
  SECTION("uint32_t") {
    SECTION("works for valid string") { REQUIRE(uint32_t{0xFEDCBA98} == binascii::from_hex_chars<uint32_t>("98BADcfE")); }
  ...
  SECTION("uint64_t") {
    SECTION("works for valid string") { REQUIRE(uint64_t{0xFEDCBA9876543210} == binascii::from_hex_chars<uint64_t>("1032547698BADcfE")); }

    SECTION("throws std::invalid_argument if the string is too short") {
      WITE_REQ_THROWS(binascii::from_hex_chars<uint64_t>("1032547698BADcf"), std::invalid_argument, "Invalid sequence length for type");
    }
