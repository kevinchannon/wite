...
TEST_CASE("read from raw byte array tests - non-io::byte buffers", "[buffer_io]") {
  SECTION("unchecked_read") {
    SECTION("returns value and next read position", "[buffer_io]") {
  ...
  SECTION("read") {
    SECTION("single value") {
      SECTION("vector buffer", "[buffer_io]") {
        const auto vec_buffer = std::vector<uint8_t>{0x67, 0x45, 0x23, 0x01, 0xEF, 0xCD, 0xAB, 0x89};
        ...
        SECTION("Big-endian") {
          ...
          SECTION("Read 2 shorts") {
            ...
            REQUIRE(uint32_t(0x2301) ==
                    io::read_with_endian<uint16_t>(std::span{std::next(vec_buffer.begin(), 2), vec_buffer.end()}, io::endian::big));
          }
      ...
      SECTION("pointers to array", "[buffer_io]") {
        constexpr auto buf_size                   = 8;
        const unsigned char data_buffer[buf_size] = {0x67, 0x45, 0x23, 0x01, 0xEF, 0xCD, 0xAB, 0x89};

        const unsigned char* buf_begin = data_buffer;
        const unsigned char* buf_end   = buf_begin + buf_size;
    ...
    SECTION("multiple-values") {
      // clang-format off
      const auto buffer = std::array<uint8_t, sizeof(uint32_t) + sizeof(uint16_t) + sizeof(std::array<uint8_t, 4>) + sizeof(bool) + sizeof(uint32_t)>{
        0x78, 0x56, 0x34, 0x12,
        0xAB, 0xCD,
        0x00, 0x01, 0x02, 0x03,
        true,
  ...
  SECTION("try_read") {
    SECTION("single value") {
      SECTION("returns value on good read") {
        const auto data = std::vector<unsigned char>{0x67, 0x45, 0xAB, 0xFF};
  ...
  SECTION("read_at") {
    ...
    SECTION("multiple values") {
      // clang-format off
      const auto buffer = std::array<unsigned char, sizeof(uint32_t) + sizeof(uint16_t) + sizeof(bool) + sizeof(uint32_t)>{
        0x78, 0x56, 0x34, 0x12,
        0xAB, 0xCD,
        true,
        0x98, 0xBA, 0xDC, 0xFE
