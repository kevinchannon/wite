...
TEMPLATE_TEST_CASE("Write values to byte arrays", "[buffer_io]", io::byte, uint8_t, int8_t, unsigned char) {
  SECTION("write") {
    SECTION("single value") {
      auto array_buffer = std::array<TestType, 10>{};
      ...
      SECTION("Little-endian") {
        SECTION("Write int at start of buffer") {
          SECTION("Dynamic endianness") {
            ...
            REQUIRE(
                std::all_of(std::next(array_buffer.begin(), 4), array_buffer.end(), [](auto&& x) { return x == TestType{0}; }));
          }
        ...
        SECTION("Write int not at start of buffer") {
          ...
          REQUIRE(
              std::all_of(array_buffer.begin(), std::next(array_buffer.begin(), 2), [](auto&& x) { return x == TestType{0}; }));
          REQUIRE(std::all_of(std::next(array_buffer.begin(), 6), array_buffer.end(), [](auto&& x) { return x == TestType{0}; }));
        }
      ...
      SECTION("Big-endian") {
        SECTION("Write int at start of buffer") {
          ...
          REQUIRE(std::all_of(std::next(array_buffer.begin(), 4), array_buffer.end(), [](auto&& x) { return x == TestType{0}; }));
        }
    ...
    SECTION("multiple values") {
      ...
      auto buffer = std::array<TestType, sizeof(a) + sizeof(b) + sizeof(c) + sizeof(d)>{};
    ...
    SECTION("range values") {
      auto buffer = std::array<TestType, 16>{};
      ...
      SECTION("static ranges") {
        ...
        constexpr auto value_size = sizeof(decltype(values)::value_type);
        auto buffer_1               = std::array<TestType, 4 * value_size>{};
  ...
  SECTION("write_at") {
    SECTION("single value") {
      auto data = std::array<TestType, 18>{};
    ...
    SECTION("multiple values") {
      ...
      auto buffer = std::array<TestType, 1 + data_size>{};
  ...
  SECTION("try_write") {
    SECTION("single value") {
      SECTION("returns number of bytes written on good write") {
        auto data = std::array<TestType, 20>{};
      ...
      SECTION("returns error on bad write") {
        auto data = std::array<TestType, 3>{};
    ...
    SECTION("multiple values") {
      ...
      auto buffer = std::array<TestType, data_size>{};
  ...
  SECTION("try_write_at") {
    SECTION("single value") {
      auto data = std::array<TestType, 12>{};
