...
template <typename T>
_WITE_CONCEPT is_buffer_readable =
    (not std::is_array_v<T>)and(common::is_pod_like<T> or (is_encoded<T> and common::is_pod_like<typename T::value_type>));

template <typename T>
_WITE_CONCEPT is_buffer_writeable =
    (not std::is_array_v<T>)and(common::is_pod_like<T> or (is_encoded<T> and common::is_pod_like<typename T::value_type>));
